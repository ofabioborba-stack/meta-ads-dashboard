-- v8 — período com data final (p_end) em todas as funções de agregação
-- drop necessário: a assinatura das funções mudou
...
create or replace function dashboard_metrics(p_start date, p_end date)
returns table (
  cliente_id text,
  cliques bigint,
  leads bigint,
...
create or replace function campaign_metrics(p_cliente_id text, p_start date, p_end date)
returns table (
  campanha text,
  alcance bigint,
  cliques bigint,
